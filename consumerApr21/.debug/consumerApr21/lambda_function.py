...
def create_language(row):
    ...
    LOG.debug(f"Found Dominant Language: {payload}")
    languages = payload['Languages']
    language_ret = ''
    # LOG.info(f"Beefooree {languages}")
    for i in range(len(languages)):
        tmp = languages[i]['LanguageCode'] + ' (' + str(round(languages[i]['Score'] * 100)) + '%)'
        language_ret += tmp
        if not i == len(languages) - 1:
            language_ret += ', '
    # LOG.info(f"Afteeeeee {language_ret}")
    return language_ret
...
def create_entities(row):
    ...
    for i in range(len(entities)):
        tmp = entities[i]['Text'] + ' - ' + entities[i]['Type'] + ' (' + str(round(entities[i]['Score'] * 100)) + '%)'
        entities_ret += tmp
...
def create_key_phrases(row):
    ...
    for i in range(len(keyPhrases)):
        tmp = keyPhrases[i]['Text'] + ' (' + str(round(keyPhrases[i]['Score'] * 100)) + '%)'
        keyPhrases_ret += tmp
        if not i == len(keyPhrases) - 1:
            keyPhrases_ret += ', '
    return keyPhrases_ret

def apply_key_phrases(df, column="wikipedia_snippit"):
    """Uses Pandas Apply to Create Sentiment Analysis"""

    df['Key Phrases'] = df[column].apply(create_key_phrases)
    return df
...
def lambda_handler(event, context):
    ...
    LOG.info(f"Dominant Language from FANG companies: {df.to_dict()}")
    ...
    # Write result to S3
    write_s3(df=df, bucket="fangsentiments", name=names)